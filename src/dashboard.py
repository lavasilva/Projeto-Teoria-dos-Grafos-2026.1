...
.chip.hub-chip.active {{ background: #f1c40f; color: #000; border-color: #f1c40f; }}
.chip.comum-chip.active {{ background: #7ec8e3; color: #000; border-color: #7ec8e3; }}
...
<main>
  ...
  <div class="filter-bar">
    <div class="filter-group">
      ...
      <div class="filter-chips" id="filter-regioes">
        <span class="chip active" data-regiao="all">Todas</span>
      </div>
    </div>
    ...
    <div class="filter-group">
      ...
      <div class="filter-chips">
        ...
        <span class="chip comum-chip" id="chip-comum" data-tipo="comum">Comum</span>
      </div>
    </div>
    <div class="filter-divider"></div>
    <div class="filter-group">
      <span class="filter-label">Grau mínimo</span>
      <div class="slider-wrap">
        <input type="range" id="slider-grau" min="1" max="7" value="1" step="1">
        <span class="slider-val" id="slider-grau-val">1</span>
      </div>
    </div>
    <button class="filter-reset" id="btn-reset-filters">↺ Resetar</button>
    <span class="filter-count" id="filter-count"><span id="fc-num">20</span> / 20 aeroportos</span>
  </div>
...
const filterState = {{ regiao: 'all', tipo: 'all', grauMin: 1 }};

function getFilteredAirports() {{
  return Object.keys(IATA_INFO).filter(iata => {{
    const info  = IATA_INFO[iata];
    const ego   = EGO.find(r => r.aeroporto === iata) || {{}};
    const grau  = parseInt(ego.grau) || 0;
    const isHub = HUBS_SET.includes(iata);
    if (filterState.regiao !== 'all' && info.regiao !== filterState.regiao) return false;
    if (filterState.tipo === 'hub'   && !isHub) return false;
    if (filterState.tipo === 'comum' && isHub)  return false;
    if (grau < filterState.grauMin) return false;
    return true;
  }});
}}

let grausChartInst = null;

function buildGrausChart() {{
  const filtered = getFilteredAirports();
  const rows = GRAUS.filter(r => filtered.includes(r.aeroporto));
  const labels = rows.map(r => r.aeroporto);
  const values = rows.map(r => parseInt(r.grau));
  const colors = rows.map(r => HUBS_SET.includes(r.aeroporto) ? '#f1c40f' : '#7ec8e3');

  if (grausChartInst) {{
    grausChartInst.data.labels = labels;
    grausChartInst.data.datasets[0].data = values;
    grausChartInst.data.datasets[0].backgroundColor = colors;
    grausChartInst.update();
    return;
  }}

  grausChartInst = new Chart(document.getElementById('chart-graus'), {{
    type: 'bar',
    data: {{
      labels,
      datasets: [{{
...
function applyFilters() {{
  const filtered = getFilteredAirports();
  document.getElementById('fc-num').textContent = filtered.length;
  buildGrausChart();
...
function rebuildTable(textFilter) {{
  ...
  rows.filter(r =>
    ...
    r.info.cidade.toLowerCase().includes(textFilter)
  ).forEach(r => {{
...
function initFilterUI() {{
  const regiaoWrap = document.getElementById('filter-regioes');
  REGIOES.forEach(r => {{
    const chip = document.createElement('span');
    chip.className = 'chip';
    chip.dataset.regiao = r.regiao;
    chip.textContent = r.regiao;
    chip.style.borderColor = REGIAO_CORES[r.regiao] || 'var(--border)';
    chip.addEventListener('click', () => {{
      filterState.regiao = r.regiao;
      regiaoWrap.querySelectorAll('.chip').forEach(c => {{
        c.classList.remove('active');
        c.style.background = '';
        c.style.color = '';
      }});
      chip.classList.add('active');
      chip.style.background = REGIAO_CORES[r.regiao] || 'var(--accent)';
      chip.style.color = '#fff';
      applyFilters();
    }});
    regiaoWrap.appendChild(chip);
  }});

  regiaoWrap.querySelector('[data-regiao="all"]').addEventListener('click', () => {{
    filterState.regiao = 'all';
    regiaoWrap.querySelectorAll('.chip').forEach(c => {{
      c.classList.remove('active');
      c.style.background = '';
      c.style.color = '';
    }});
    regiaoWrap.querySelector('[data-regiao="all"]').classList.add('active');
    applyFilters();
  }});

  ['chip-todos','chip-hub','chip-comum'].forEach(id => {{
    document.getElementById(id).addEventListener('click', () => {{
      const tipo = {{ 'chip-todos':'all', 'chip-hub':'hub', 'chip-comum':'comum' }}[id];
      filterState.tipo = tipo;
      ['chip-todos','chip-hub','chip-comum'].forEach(cid => document.getElementById(cid).classList.remove('active'));
  ...
  slider.max = Math.max(...GRAUS.map(g => parseInt(g.grau)));
